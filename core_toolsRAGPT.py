...
import pandas as pd
from transformers import AutoModel, AutoProcessor, BertTokenizer, ViltImageProcessor, ViltModel
import json
...
def _first_existing_path(*paths):
    ...
    raise FileNotFoundError(
        "None of the expected files exist: " + ", ".join(paths)
    )
...
def init_data_mmimdb():
    split_path = _first_existing_path(
        'dataset/mmimdb/split.json',
        'dataset/mmimdb/meta_data/split.json',
    )
    data_split = json.load(open(split_path))
    ...
    for split in tqdm(['train', 'dev', 'test']):
        ...
        for item_id in item_id_list:
            metadata_path = f'dataset/mmimdb/meta_data/{item_id}.json'
            if not os.path.exists(metadata_path):
                raise FileNotFoundError(
                    f"MMIMDb metadata file not found: {metadata_path}. "
                    "Please put the raw MMIMDb JSON metadata files under dataset/mmimdb/meta_data/."
                )
            json_file = json.load(open(metadata_path))
            try:
...
def init_data_food101():
    class_idx_path = _first_existing_path(
        'dataset/food101/class_idx.json',
        'dataset/food101/meta_data/class_idx.json',
    )
...
class MemoryBankGenerator(torch.nn.Module):
    def __init__(self, cfg):
        ...
        # 用 ViLT 的 embedding 层来编码 img和txt
        ...
        requested_max_text_len = cfg.data_para.max_text_len
        vilt_max_text_len = self._get_vilt_max_text_len(pretrained_vilt)
        self.max_text_len = min(requested_max_text_len, vilt_max_text_len)
        if self.max_text_len != requested_max_text_len:
            print(
                "Requested max_text_len="
                f"{requested_max_text_len}, but dandelin/vilt-b32-mlm only supports "
                f"{vilt_max_text_len} text positions. Using max_text_len={self.max_text_len} "
                "for memory bank generation."
            )
    ...
    def _get_vilt_max_text_len(self, pretrained_vilt):
        position_embeddings = pretrained_vilt.embeddings.text_embeddings.position_embeddings
        return position_embeddings.num_embeddings

    def _freeze(self):
        for param in self.embedding_layer.parameters():
            param.requires_grad = False

    # 用 ViLT 的 embedding 层同时编码文本和图像
    ...
    def _process_batch(self, df, start_idx, end_idx):
        ...
        pending = []
        for text, item_id in zip(texts.tolist(), ids.tolist()):
            text_path = f'dataset/memory_bank/{self.dataset}/text/{item_id}.npy'
            image_path = f'dataset/memory_bank/{self.dataset}/image/{item_id}.npy'
            if not os.path.exists(text_path) or not os.path.exists(image_path):
                pending.append((text, item_id))

        if not pending:
            return

        texts = [item[0] for item in pending]
        ids = [item[1] for item in pending]
    ...
    def run(self):
        for i in tqdm(range(0, len(self.df_train), self.batch_size), desc="MemoryBank train"):
            start_idx = i
            end_idx = min(i + self.batch_size, len(self.df_train))
            self._process_batch(self.df_train, start_idx, end_idx)
         
        if self.dataset != "food101":
            for i in tqdm(range(0, len(self.df_valid), self.batch_size), desc="MemoryBank valid"):
                start_idx = i
                end_idx = min(i + self.batch_size, len(self.df_valid))
                self._process_batch(self.df_valid, start_idx, end_idx)

        for i in tqdm(range(0, len(self.df_test), self.batch_size), desc="MemoryBank test"):
            start_idx = i
            end_idx = min(i + self.batch_size, len(self.df_test))
            self._process_batch(self.df_test, start_idx, end_idx)
...
class MCR():
    ...
    def _compute_similarity_in_batches(self, query_vectors, memory_bank, memory_bank_id, memory_bank_label):
        ...
        sims_list = []
        r_label_list = []
        top_k = min(self.top_k, memory_bank.size(0))
        with torch.no_grad():
            memory_bank = F.normalize(memory_bank.float(), p=2, dim=-1)
            memory_bank_t = memory_bank.transpose(0, 1).contiguous()
            for i in tqdm(range(0, len(query_vectors), self.batch_size)):
                batch = query_vectors[i:i+self.batch_size]
                batch = F.normalize(batch.float(), p=2, dim=-1)
                similarity = torch.matmul(batch, memory_bank_t)
                sim_scores, top_k_id = torch.topk(similarity, k=top_k, dim=-1)
    ...
    # 给 train/test/valid 全部样本生成 q_i/q_t
    ...
    # 构造检索库 r_v_i/r_v_t，然后分别做图像和文本检索
...
def load_model(is_baseline=False, **kargs):
    raise NotImplementedError(
        "DEPMultiRAG only reuses the RAGPT data initialization and retrieval utilities. "
        "Loading the original RAGPT classifier model is not wired in this directory."
    )

def get_dataset(dataset_name: str, model_name: str, **kargs):
    try:
        module = importlib.import_module(f"dataloaderRAGPT.{model_name}_{dataset_name}_dataset")
    except ModuleNotFoundError:
        module = importlib.import_module(
            f"promptlearn.DEPMultiRAG.dataloaderRAGPT.{model_name}_{dataset_name}_dataset"
        )
...
# 将检索结果r_t_list，r_i_list，r_l_list整理成三元组向量
class Collator:
